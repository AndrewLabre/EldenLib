...
#include "../headers/Mahoraga.hpp"
#include "../headers/BezierAttack.hpp"


// UTILS
void CheckCollision(Mahoraga& enemy)
{
...
void Mahoraga::Update(float delta_time) {
    ...
    if (GetState() == &readying1 || GetState() == &readying2 || GetState() == &readying3)
    {
...
MahoragaState* Mahoraga::GetState() {
    return current_state;
}
void Mahoraga::TakeDmg() {
    if(iFrameTimer <= 0.0f)
    {
        hp -= incoming_dmg;
        iFrameTimer = iFrames;
